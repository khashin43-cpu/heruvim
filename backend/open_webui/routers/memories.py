...
class ConfirmMemoryResponse(BaseModel):
    memory: MemoryModel
    confirmed_at: int
...
@router.post('/{memory_id}/confirm', response_model=ConfirmMemoryResponse)
async def confirm_memory_by_id(
    memory_id: str,
    request: Request,
    user=Depends(get_verified_user),
):
    """Mark a remembered fact as reviewed by its owner."""
    await check_memories_permission(user)
    ...
    memory = await Memories.update_memory_by_id_and_user_id(
        ...
        user.id,
        None,
        meta={'confirmed': True, 'confirmed_at': confirmed_at},
    )
    if memory is None:
        raise HTTPException(status_code=404, detail=ERROR_MESSAGES.NOT_FOUND)
    ...
    return ConfirmMemoryResponse(memory=memory, confirmed_at=confirmed_at)
