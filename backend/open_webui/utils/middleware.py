...
def _is_heruvim_tool_server(server: dict) -> bool:
    ...
    candidates = [
        ...
        info.get('id'),
        info.get('name'),
        (server.get('openapi') or {}).get('info', {}).get('title'),
    ]
    return any(
        isinstance(value, str) and (value.startswith('heruvim-') or value.startswith('ХЕРУВИМ'))
        for value in candidates
    )


def _heruvim_uses_ragflow_file_context() -> bool:
    return bool(HERUVIM_RAGFLOW_ENABLED and HERUVIM_RAGFLOW_SYNC_ATTACHMENTS and not HERUVIM_OPENWEBUI_FILE_CONTEXT)


_HERUVIM_DOCUMENT_ACTION_RE = re.compile(
    ...
    r'создай|создать|сформируй|подготовь|сохрани|сохранить|экспорт|конверт|'
    r'метаданн|ocr|распознай|сделай\s+поисковым|можешь\s+.*редакт)\w*\b'
)
_HERUVIM_PDF_RE = re.compile(r'(?i)\b(pdf|пдф|скан|ocr)\b')
_HERUVIM_DOCX_RE = re.compile(r'(?i)\b(docx|word|ворд)\b')
_HERUVIM_OFFICE_RE = re.compile(r'(?i)\b(xlsx|excel|эксел|pptx|powerpoint|презентац)\w*\b')


def _heruvim_connection_kind(connection: dict) -> str:
...
def _heruvim_requested_tool_kinds(prompt: str) -> set[str]:
    if not prompt or not _HERUVIM_DOCUMENT_ACTION_RE.search(prompt):
        return set()
    if _HERUVIM_OFFICE_RE.search(prompt):
        return {'gateway'}
    if _HERUVIM_PDF_RE.search(prompt):
        return {'pdf'}
    if _HERUVIM_DOCX_RE.search(prompt):
        return {'docx'}
    # A generic document edit may target either Word or PDF. Keep both
    # editors available and let the model select from the attached file type.
    return {'docx', 'pdf'}


async def _inject_heruvim_openapi_tool_servers(metadata: dict, prompt: str = '') -> None:
    if metadata.get('tool_servers'):
        return

    requested_kinds = _heruvim_requested_tool_kinds(prompt)
    ...
    heruvim_connections = [
        ...
        and _is_heruvim_tool_server(connection)
        and _heruvim_connection_kind(connection) in requested_kinds
    ]
    if not heruvim_connections:
        return

    tool_servers = await get_tool_servers_data(heruvim_connections)
    tool_servers = [server for server in tool_servers if _is_heruvim_tool_server(server)]
    if tool_servers:
        system_prompt = (
            'Heruvim document tools are available. Current chat attachments and indexed RAGFlow corpora are different sources. '
            'Use local attachment paths with MCP tools for files attached to the current chat. '
            'Use RAGFlow search only for indexed knowledge-base/corpus/archive search. '
            'You can search RAGFlow documents, read PDF/DOCX/TXT files, '
            'create DOCX files, replace exact text in DOCX files, run OfficeCLI for Office documents, '
            'For DOCX creation use the exact heruvim_docx_create operation with output_path, title, and paragraphs. '
            'extract PDF text blocks and coordinates, replace text in text-layer PDFs, redact PDF text, '
            'add text to PDFs, make scanned PDFs searchable with OCRmyPDF/Tesseract, replace visual text in '
            'scanned PDFs using OCR coordinates, and perform PDF page operations such as extract pages, '
            'delete pages, rotate pages, merge PDFs, and set PDF metadata. Do not claim these tools are unavailable.'
        )
        for server in tool_servers:
            server.setdefault('system_prompt', system_prompt)
        metadata['tool_servers'] = tool_servers
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    await _inject_heruvim_openapi_tool_servers(metadata, prompt or '')
    ...
    if payload_tools is None:
        ...
        if use_builtin_tools:
            ...
            builtin_model = model
            builtin_extra_params = extra_params
            if heruvim_ragflow_files:
                builtin_model = copy.deepcopy(model)
                builtin_meta = builtin_model.setdefault('info', {}).setdefault('meta', {})
                builtin_flags = dict(builtin_meta.get('builtinTools') or {})
                builtin_flags.update({'files': False, 'knowledge': False})
                builtin_meta['builtinTools'] = builtin_flags
...
async def streaming_chat_response_handler(response, ctx):
    ...
    if event_emitter:
        ...
        async def response_handler(response, events):
            ...
            heruvim_artifacts = [
                artifact
                for artifact in (getattr(request.state, 'heruvim_artifacts', []) or [])
                if isinstance(artifact, dict) and artifact.get('id')
            ]
            if heruvim_artifacts:
                existing_files = list(message.get('files', []) if message else [])
                existing_ids = {
                    str(file.get('id') or file.get('url') or '')
                ...
                generated_files = [
                    {
                        ...
                        'meta': {'content_type': artifact.get('content_type')},
                        'preview_url': artifact.get('preview_url'),
                        'download_url': artifact.get('download_url'),
                    }
                    for artifact in heruvim_artifacts
                    if str(artifact['id']) not in existing_ids
                ]
                if generated_files:
                    message_files = existing_files + generated_files
                    if save_to_chat:
                        saved_files = await Chats.add_message_files_by_id_and_message_id(
                            metadata['chat_id'],
                            metadata['message_id'],
                            generated_files,
                        )
                        if saved_files is not None:
                            message_files = saved_files
