...
# ХЕРУВИМ uses an explicit LLM endpoint as the chat model. RAGFlow is kept as
# the document search / ingestion engine. For backward compatibility, a RAGFlow
# chat assistant can still be exposed as the model when no direct LLM endpoint
# is configured.
HERUVIM_LLM_BASE_URL = os.getenv(
    'HERUVIM_LLM_BASE_URL',
    os.getenv('HERUVIM_LLM_API_BASE_URL', ''),
).strip().rstrip('/')
HERUVIM_LLM_API_KEY = os.getenv(
    'HERUVIM_LLM_API_KEY',
    os.getenv('HERUVIM_LLM_KEY', ''),
).strip()
HERUVIM_LLM_MODEL = os.getenv(
    'HERUVIM_LLM_MODEL',
    os.getenv('HERUVIM_LLM_MODEL_NAME', ''),
).strip()
HERUVIM_LLM_DISPLAY_NAME = os.getenv('HERUVIM_LLM_DISPLAY_NAME', 'ХЕРУВИМ').strip() or 'ХЕРУВИМ'
HERUVIM_LLM_ENABLED = bool(HERUVIM_LLM_BASE_URL and HERUVIM_LLM_MODEL and HERUVIM_LLM_API_KEY)
HERUVIM_LLM_ENABLE_THINKING = os.getenv('HERUVIM_LLM_ENABLE_THINKING', 'False').lower() == 'true'
HERUVIM_LLM_THINKING_BUDGET = max(
    0,
    min(8192, int(os.getenv('HERUVIM_LLM_THINKING_BUDGET', '512'))),
)

HERUVIM_RAGFLOW_BASE_URL = os.getenv('HERUVIM_RAGFLOW_BASE_URL', 'http://127.0.0.1:9380').rstrip('/')
HERUVIM_RAGFLOW_API_KEY = os.getenv('HERUVIM_RAGFLOW_API_KEY', '').strip()
HERUVIM_RAGFLOW_CHAT_ID = os.getenv('HERUVIM_RAGFLOW_CHAT_ID', '').strip().strip('/')
HERUVIM_RAGFLOW_ENABLED = bool(HERUVIM_RAGFLOW_API_KEY and HERUVIM_RAGFLOW_CHAT_ID)
HERUVIM_RAGFLOW_SYNC_ATTACHMENTS = os.getenv('HERUVIM_RAGFLOW_SYNC_ATTACHMENTS', 'True').lower() == 'true'
HERUVIM_OPENWEBUI_INTERNAL_FILE_PROCESSING = (
    os.getenv('HERUVIM_OPENWEBUI_INTERNAL_FILE_PROCESSING', 'False').lower() == 'true'
)
HERUVIM_OPENWEBUI_FILE_CONTEXT = os.getenv('HERUVIM_OPENWEBUI_FILE_CONTEXT', 'False').lower() == 'true'
HERUVIM_RAGFLOW_DATASET_IDS = [
    value.strip()
    for value in os.getenv('HERUVIM_RAGFLOW_DATASET_IDS', '').replace(',', ';').split(';')
    if value.strip()
]
HERUVIM_RAGFLOW_SYNC_WORKERS = max(1, int(os.getenv('HERUVIM_RAGFLOW_SYNC_WORKERS', '2')))
HERUVIM_RAGFLOW_SYNC_POLL_SECONDS = max(1.0, float(os.getenv('HERUVIM_RAGFLOW_SYNC_POLL_SECONDS', '3')))
HERUVIM_RAGFLOW_SYNC_TIMEOUT_SECONDS = max(30, int(os.getenv('HERUVIM_RAGFLOW_SYNC_TIMEOUT_SECONDS', '1200')))
HERUVIM_REQUIRE_SOURCE_VERIFICATION = os.getenv('HERUVIM_REQUIRE_SOURCE_VERIFICATION', 'True').lower() == 'true'
HERUVIM_RAGFLOW_DIRECT_CHAT_FALLBACK = os.getenv('HERUVIM_RAGFLOW_DIRECT_CHAT_FALLBACK', 'False').lower() == 'true'
HERUVIM_RAGFLOW_AUTO_RETRIEVAL = os.getenv('HERUVIM_RAGFLOW_AUTO_RETRIEVAL', 'True').lower() == 'true'
HERUVIM_RAGFLOW_AUTO_RETRIEVAL_MODE = os.getenv('HERUVIM_RAGFLOW_AUTO_RETRIEVAL_MODE', 'auto').strip().lower()
HERUVIM_RAGFLOW_RETRIEVAL_PAGE_SIZE = max(1, min(30, int(os.getenv('HERUVIM_RAGFLOW_RETRIEVAL_PAGE_SIZE', '8'))))
HERUVIM_RAGFLOW_RETRIEVAL_SIMILARITY_THRESHOLD = max(
    0.0,
    min(1.0, float(os.getenv('HERUVIM_RAGFLOW_RETRIEVAL_SIMILARITY_THRESHOLD', '0.2'))),
)
HERUVIM_RAGFLOW_RETRIEVAL_VECTOR_WEIGHT = max(
    0.0,
    min(1.0, float(os.getenv('HERUVIM_RAGFLOW_RETRIEVAL_VECTOR_WEIGHT', '0.3'))),
)
HERUVIM_RAGFLOW_FULL_DOCUMENTS = os.getenv('HERUVIM_RAGFLOW_FULL_DOCUMENTS', 'True').lower() == 'true'
HERUVIM_RAGFLOW_FULL_DOCUMENT_LIMIT = max(0, min(5, int(os.getenv('HERUVIM_RAGFLOW_FULL_DOCUMENT_LIMIT', '2'))))
HERUVIM_RAGFLOW_FULL_DOCUMENT_MAX_CHARS = max(
    4000,
    min(120000, int(os.getenv('HERUVIM_RAGFLOW_FULL_DOCUMENT_MAX_CHARS', '60000'))),
)
HERUVIM_DEFAULT_MODEL = (
    HERUVIM_LLM_MODEL
    if HERUVIM_LLM_ENABLED
    else ('model' if HERUVIM_RAGFLOW_ENABLED and HERUVIM_RAGFLOW_DIRECT_CHAT_FALLBACK else None)
)

if HERUVIM_LLM_ENABLED:
    # Remove the unused upstream OpenAI placeholder on a fresh installation.
    if OPENAI_API_BASE_URLS == ['https://api.openai.com/v1'] and OPENAI_API_KEYS == ['']:
        OPENAI_API_BASE_URLS = []
        OPENAI_API_KEYS = []

    if HERUVIM_LLM_BASE_URL in OPENAI_API_BASE_URLS:
        connection_index = OPENAI_API_BASE_URLS.index(HERUVIM_LLM_BASE_URL)
        while len(OPENAI_API_KEYS) <= connection_index:
            OPENAI_API_KEYS.append('')
        OPENAI_API_KEYS[connection_index] = HERUVIM_LLM_API_KEY
    else:
        connection_index = len(OPENAI_API_BASE_URLS)
        OPENAI_API_BASE_URLS.append(HERUVIM_LLM_BASE_URL)
        OPENAI_API_KEYS.append(HERUVIM_LLM_API_KEY)
    OPENAI_API_CONFIGS[str(connection_index)] = {
        **OPENAI_API_CONFIGS.get(str(connection_index), {}),
        'enable': True,
        'provider': 'openai',
...
elif HERUVIM_LLM_BASE_URL or HERUVIM_LLM_MODEL or HERUVIM_LLM_API_KEY:
    log.warning('HERUVIM LLM connection is incomplete: base URL, model and API key are required')

if HERUVIM_RAGFLOW_ENABLED and not HERUVIM_LLM_ENABLED and HERUVIM_RAGFLOW_DIRECT_CHAT_FALLBACK:
    # Remove the unused upstream OpenAI placeholder on a fresh installation.
    if OPENAI_API_BASE_URLS == ['https://api.openai.com/v1'] and OPENAI_API_KEYS == ['']:
        OPENAI_API_BASE_URLS = []
        OPENAI_API_KEYS = []
    ...
    if ragflow_url in OPENAI_API_BASE_URLS:
        connection_index = OPENAI_API_BASE_URLS.index(ragflow_url)
        while len(OPENAI_API_KEYS) <= connection_index:
            OPENAI_API_KEYS.append('')
        OPENAI_API_KEYS[connection_index] = HERUVIM_RAGFLOW_API_KEY
    else:
        connection_index = len(OPENAI_API_BASE_URLS)
        OPENAI_API_BASE_URLS.append(ragflow_url)
        OPENAI_API_KEYS.append(HERUVIM_RAGFLOW_API_KEY)
    OPENAI_API_CONFIGS[str(connection_index)] = {
        **OPENAI_API_CONFIGS.get(str(connection_index), {}),
        'enable': True,
        'provider': 'ragflow',
...
elif not HERUVIM_LLM_ENABLED and not HERUVIM_RAGFLOW_ENABLED and (HERUVIM_RAGFLOW_API_KEY or HERUVIM_RAGFLOW_CHAT_ID):
    log.warning('HERUVIM RAGFlow connection is incomplete: both API key and chat ID are required')
elif not HERUVIM_LLM_ENABLED and HERUVIM_RAGFLOW_ENABLED and not HERUVIM_RAGFLOW_DIRECT_CHAT_FALLBACK:
    log.warning(
        'HERUVIM RAGFlow is configured for document ingestion/search, but no chat model is registered because '
        'HERUVIM_LLM_* is incomplete and HERUVIM_RAGFLOW_DIRECT_CHAT_FALLBACK is disabled'
    )
...
DEFAULT_MODELS = os.getenv('DEFAULT_MODELS', HERUVIM_DEFAULT_MODEL)
