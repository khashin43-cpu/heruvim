    ...
    async def __event_emitter__(event_data):
        ...
        if save_to_chat:
            ...
            if event_type == 'status':
                ...
                incoming_files = event_data.get('data', {}).get('files', [])
                existing_files = message.get('files', []) if message else []
                files = []
                seen = set()
                for file in [*incoming_files, *existing_files]:
                    if not isinstance(file, dict):
                        continue
                    key = (
                        str(file.get('type') or ''),
                        str(file.get('id') or file.get('url') or file.get('name') or ''),
                    )
                    if key in seen:
                        continue
                    seen.add(key)
