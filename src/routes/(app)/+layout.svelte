<script lang="ts">
	...
	let redirectingUserRoute = false;

	const isSimpleUserRoute = (pathname: string) =>
		pathname === '/' || pathname === '/ingestion' || pathname.startsWith('/c/');

	const enforceSimpleUserRoute = async () => {
		if ($user?.role !== 'user' || isSimpleUserRoute($page.url.pathname) || redirectingUserRoute) return;
		redirectingUserRoute = true;
		await goto($page.url.pathname.startsWith('/workspace/knowledge') ? '/ingestion' : '/', {
			replaceState: true
		});
		redirectingUserRoute = false;
	};
	...
	const setUserSettings = async (cb?: () => Promise<void>) => {
		...
		setTextScale($settings?.textScale ?? 1.06);
	...
	const openSettingsFromUrl = async () => {
		...
		showSettings.set($user?.role === 'admin' ? requestedSettings : 'account');
			...
			document.addEventListener('keydown', async (event) => {
				...
				if (shortcut === Shortcut.SEARCH) {
					...
					showControls.set(!$showControls);
						...
						event.preventDefault();
						showSettings.set($user?.role === 'admin' ? !$showSettings : 'account');
					} else if (shortcut === Shortcut.SHOW_SHORTCUTS) {
						console.log('Shortcut triggered: SHOW_SHORTCUTS');
						event.preventDefault();
						showSettings.set($user?.role === 'admin' ? 'shortcuts' : 'account');
