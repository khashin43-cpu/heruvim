<script lang="ts">
	...
	export let executiveMode = false;
	...
	$: showToolsButton = $_user?.role === 'admin' && (($tools ?? []).length > 0 || ($toolServers ?? []).length > 0);

	let showSkillsButton = false;
	$: showSkillsButton = $_user?.role === 'admin' && ($skills ?? []).some((skill) => skill.is_active);
							...
							<div class=" flex justify-between mt-0.5 mb-2 mx-0.5 max-w-full" dir="ltr">
								<div class="ml-1 self-end flex items-center flex-1 min-w-0">
									...
									<div class="flex flex-1 items-center min-w-0 overflow-x-auto scrollbar-none">
										...
										{#if $_user?.role === 'admin' && selectedModelIds.length === 1 && $models.find((m) => m.id === selectedModelIds[0])?.has_user_valves}
								...
								<div class="self-end flex space-x-1 mr-1 shrink-0 gap-[0.5px]">
									{#if !executiveMode || selectedModelIds.length === 0}
