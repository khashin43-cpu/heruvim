<script lang="ts">
	...
	import SparklesIcon from './Sidebar/icons/Sparkles.svelte';
	import Sidebar from '../icons/Sidebar.svelte';
	import WorkspaceIcon from './Sidebar/icons/Workspace.svelte';
	import ArrowPathIcon from '../icons/ArrowPath.svelte';
	...
	const DEFAULT_PINNED_ITEMS = ['ingestion', 'memory', 'notes', 'workspace'];
	...
	$: adminPinnedItems = $settings?.pinnedMenuItems ?? DEFAULT_PINNED_ITEMS;
	$: pinnedItems = isAdmin
		? adminPinnedItems.includes('memory')
			? adminPinnedItems
			: [...adminPinnedItems, 'memory']
		: ['ingestion', 'memory'];

	const isMenuItemVisible = (id) => {
		if ($user?.role !== 'admin') {
			if (id === 'ingestion') return true;
			if (id === 'memory') {
				return (
		...
		switch (id) {
			case 'ingestion':
				return true;
			case 'memory':
				return $config?.features?.enable_memories ?? false;
			case 'notes':
				return (
	...
	const getMenuItemMeta = (id) => {
		const items = {
			ingestion: { label: 'База знаний', href: '/ingestion', iconType: 'ingestion' },
			memory: { label: 'Что помнит ХЕРУВИМ', href: '/memories', iconType: 'memory' },
	...
	onMount(async () => {
		...
		showSidebar.set(!$mobile ? (isAdmin ? localStorage.sidebar === 'true' : true) : false);
...
{#if !$mobile && !$showSidebar}
	...
	>
		...
		>
			<div class="pb-1">
				...
				>
					...
					>
						...
						>
							<img
								src="{WEBUI_BASE_URL}/heruvim-mark.svg"
			...
			<div class="-gap-0.5">
				<div class="">
						<Tooltip content={isAdmin ? $i18n.t('New Chat') : 'Новый разговор'} placement="right">
						<a
							...
							on:click={async (e) => {
								...
								aria-label={isAdmin ? $i18n.t('New Chat') : 'Новый разговор'}
						>
							<div
								class=" self-center flex size-[30px] items-center justify-center rounded-lg transition group-hover:bg-gray-50 dark:group-hover:bg-gray-900"
							>
				...
				<div>
						<Tooltip content={isAdmin ? $i18n.t('Search') : 'Найти разговор'} placement="right">
						<button
							class=" cursor-pointer flex size-8 items-center justify-center transition group"
							on:click={(e) => {
								e.stopImmediatePropagation();
							...
							draggable="false"
								aria-label={isAdmin ? $i18n.t('Search') : 'Найти разговор'}
						>
							<div
								class=" self-center flex size-[30px] items-center justify-center rounded-lg transition group-hover:bg-gray-50 dark:group-hover:bg-gray-900"
							>
						...
						<div class="">
							<Tooltip content={$i18n.t(meta.label)} placement="right">
								...
								>
									...
									>
							{#if itemId === 'ingestion'}
								<DatabaseIcon className="size-4" strokeWidth="1.5" />
	...
	<div
		...
		class="h-screen max-h-[100dvh] min-h-screen select-none {$showSidebar
			...
			: ' bg-transparent z-0 '} {$isApp
					? `ml-[4.5rem] md:ml-0 `
					: ' transition-all duration-300 '} shrink-0 text-gray-700 dark:text-gray-300 {isAdmin ? 'text-[13px]' : 'text-[15px]'} leading-5 fixed top-0 left-0 overflow-x-hidden
        "
		...
		>
			...
			>
				<div class="pb-1">
					<div class="px-1 flex justify-center text-gray-700 dark:text-gray-300">
						...
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
									<div class="self-center leading-5">{isAdmin ? $i18n.t('New Chat') : 'Новый разговор'}</div>
							</div>
					...
					<div class="px-1 flex justify-center text-gray-700 dark:text-gray-300">
						...
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class="self-center leading-5">{isAdmin ? $i18n.t('Search') : 'Найти разговор'}</div>
							</div>
							...
							{#if meta && isMenuItemVisible(itemId)}
								...
								>
									...
									>
										...
										<div class="self-center leading-5">{$i18n.t(meta.label)}</div>
										</div>
									</a>
								</div>
							{/if}
					...
					{#if isAdmin && ($models ?? []).length > 0 && (($settings?.pinnedModels ?? []).length > 0 || $config?.default_pinned_models)}
					<SidebarSection
						id="sidebar-models"
				...
				{/if}
					...
					{#if isAdmin && ($config?.features?.enable_notes ?? false) && $pinnedNotes.length > 0}
					<SidebarSection
				...
				{/if}
					...
					{#if isAdmin && $config?.features?.enable_channels}
					<SidebarSection
						id="sidebar-channels"
				...
				{/if}
					...
					{#if isAdmin && $config?.features?.enable_folders}
					<SidebarSection
						id="sidebar-folders"
				...
				<SidebarSection
					id="sidebar-chats"
					className="mt-0.5"
						name={isAdmin ? $i18n.t('Chats') : 'История разговоров'}
					on:change={async (e) => {
						selectedFolder.set(null);
					}}
					on:import={(e) => {
