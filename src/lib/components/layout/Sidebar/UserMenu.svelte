<script lang="ts">
	...
	const DEFAULT_PINNED_ITEMS = ['ingestion', 'notes', 'workspace'];
	...
	<div slot="content">
		<DropdownMenu className="{className} font-sans text-xs">
			...
			<div class="flex items-center w-full">
				...
				>
					<div class="self-center">
						<ArrowPathIcon className="size-3.5" strokeWidth="1.5" />
					</div>
						<div class="self-center truncate">База знаний</div>
					</a>
					{#if shiftKey && role === 'admin'}
					<Tooltip
						content={isPinned('ingestion')
					...
					>
						<button
							...
							on:click|preventDefault|stopPropagation={() => togglePin('ingestion')}
						>
							{#if isPinned('ingestion')}
			...
			{/if}
				...
				{#if role === 'admin' && ($config?.features?.enable_notes ?? false)}
				<div class="flex items-center w-full">
					<a
						href="/notes"
						draggable="false"
			...
			{/if}
				...
				{#if role === 'admin' && $config?.features?.enable_calendar}
				<div class="flex items-center w-full">
					<a
						href="/calendar"
						draggable="false"
			...
			{/if}
				...
				{#if role === 'admin' && $config?.features?.enable_automations}
				<div class="flex items-center w-full">
					<a
						href="/automations"
						draggable="false"
			...
			<button
				...
				on:click={async () => {
					show = false;
						...
						await showSettings.set(role === 'admin' ? true : 'account');
			...
			>
				<div class="self-center">
					...
					<div class="self-center truncate">{role === 'admin' ? $i18n.t('Settings') : 'Профиль и настройки'}</div>
			</button>
